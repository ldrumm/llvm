...
clang::QualType getBaseType(StringView Name, clang::ASTContext *AST,
  ...
  if (Name == "void")
    ...
    auto *ED =
        EnumDecl::Create(*AST, DC, SourceLocation(), SourceLocation(),
  ...
  FunctionDecl *createKernelDecl(
    ...
    FunctionDecl *FD = FunctionDecl::Create(
        *AST, AST->getTranslationUnitDecl(), SourceLocation{},
        ...
        AST->getTrivialTypeSourceInfo(AST->VoidTy), SC_None,
    ...
    for (auto &QT : ArgTys) {
      ...
      auto *NewParam = ParmVarDecl::Create(*AST, FD, SourceLocation(),
    ...
    for (auto &TemplateArgQT : TemplateArgTys) {
      ...
      TTPD->setDefaultArgument(
          AST->getTrivialTypeSourceInfo(TemplateArgQT));
  ...
  handleLeafTypeNode(const NameType *NT,
    ...
    if (KNN != PossibleKinds.end()) {
      ...
      if (NestedNamesQTMap.count(N) == 0) {
        ...
        if (!SpvNamespace)
          SpvNamespace = NamespaceDecl::Create(
              *AST, AST->getTranslationUnitDecl(), false, SL, SL,
              &AST->Idents.get("__spv", tok::TokenKind::identifier),
              nullptr, false);
        ...
        auto *ED = EnumDecl::Create(*AST, RD, SourceLocation(),
...
public:
  TargetTypeReplacements() {
    ...
    // Make replaced char functions clones of explicit signed or unsigned or long
    // long variant
...
public:
  LibCLCRemangler() : AST(nullptr), Ctx(), Replacements() {}
  ...
  void Initialize(ASTContext &C) override {
    ...
    std::unique_ptr<llvm::Module> const M =
        ExitOnErr(Expected<std::unique_ptr<llvm::Module>>(
            parseIR(Buff.get()->getMemBufferRef(), Err, Ctx)));
...
private:
  ...
  createCloneFromMap(llvm::Module *M, std::string OriginalName,
    ...
    Remangler ATR{AST, FunctionTree, TypeReplacements};
  ...
  void handleModule(llvm::Module *M) {
    ...
    if (TestRun) {
      if (Verbose)
        errs() << "Successfully processed: " << N << " functions.\n";
      return;
    }
